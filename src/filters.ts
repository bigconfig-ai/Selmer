...
export function parseNumber(value: unknown): number {
  ...
  if (Number.isNaN(n)) throw new Error(`Expected '${String(value)}' to be a number.`);
  return n;
}

function throwWhenExpectingSeqable(x: unknown, msg?: string): void {
  if (isNil(x)) return;
  try {
    seq(x);
  } catch {
...
function throwWhenExpectingNumber(x: unknown, msg?: string): void {
  if (typeof x !== "number") throw new Error(msg ?? `Expected '${isNil(x) ? "nil" : String(x)}' to be a number.`);
}
...
function formatNumberLikeClojure(value: number): string | number {
  if (Number.isInteger(value)) return value;
  return Number(value.toFixed(12)).toString();
}

function range(end: number, start?: number, step?: number): number[] {
  const actualStart = start ?? 0;
  const actualStep = step ?? 1;
  const result: number[] = [];
  if (actualStep === 0) throw new Error("step must not be 0");
  if (actualStep > 0) {
    for (let i = actualStart; i < end; i += actualStep) result.push(i);
  } else {
    for (let i = actualStart; i > end; i += actualStep) result.push(i);
  }
  return result;
...
function localeFromArgs(locale?: unknown, country?: unknown): string | undefined {
  if (!locale) return undefined;
  return country ? `${locale}-${country}` : String(locale).replace("_", "-");
}

function numberFormat(n: number, fmt: string, locale?: string): string {
...
function formatDate(dateLike: unknown, fmt: string, locale?: unknown): string | undefined {
  if (isNil(dateLike)) return undefined;
  let wrapQuotes = false;
  if (fmt.startsWith('"') && fmt.endsWith('"')) {
    fmt = fmt.slice(1, -1);
    wrapQuotes = true;
  }
  const date = dateLike instanceof Date ? dateLike : new Date(dateLike as string | number);
  if (Number.isNaN(date.getTime())) throw new Error(`${String(dateLike)} is not a valid date format.`);
  const loc = localeFromArgs(locale) ?? undefined;

  const aliases: Record<string, () => string> = {
    shortTime: () => `${pad(date.getHours())}:${pad(date.getMinutes())}`,
    shortDate: () => {
      if (loc?.startsWith("zh")) return `${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`;
    ...
    longDate: () => {
      if (loc?.startsWith("zh")) return `${date.getFullYear()}年${date.getMonth() + 1}月${date.getDate()}日`;
...
function formatJavaDatePattern(date: Date, fmt: string, locale?: string): string {
...
function getFilterValue(value: unknown, key: unknown, defaultValue?: unknown): unknown {
  const found = getAccessor(value, typeof key === "string" && key.startsWith(":") ? key.slice(1) : (key as string | number));
  return found === undefined || found === null ? defaultValue : found;
}

const builtIns: Record<string, FilterFunction> = {
  str: (x) => cljStr(x),
  ...
  "abbr-left": (s) => ({ ...(typeof s === "object" && s !== null ? (s as object) : { s }), abbrPosition: "left" }),
  "abbr-middle": (s) => ({ ...(typeof s === "object" && s !== null ? (s as object) : { s }), abbrPosition: "middle" }),
  "abbr-right": (s) => ({ ...(typeof s === "object" && s !== null ? (s as object) : { s }), abbrPosition: "right" }),
  "abbr-ellipsis": (s, ellipsis) => ({
    ...(typeof s === "object" && s !== null ? (s as object) : { s }),
    abbrEllipsis: String(ellipsis)
  }),
  abbreviate: (input, maxWidthArg, abbreviatedWidthArg) => {
    const obj = typeof input === "object" && input !== null && "s" in input ? (input as Record<string, unknown>) : { s: input };
    ...
    if (maxWidth < abbreviatedWidth) throw new Error(`Maximum width ${maxWidth} can't be shorter than abbreviated width ${abbreviatedWidth}`);
    if (abbreviatedWidth < ellipsis.length) throw new Error(`Length ${ellipsis.length} of ellipsis '${ellipsis}' can't be bigger than abbreviated width ${abbreviatedWidth}`);
  ...
  add: (x, y, ...rest) => {
    const args: unknown[] = [String(x), y, ...rest];
    if (args.every(isNumericString)) return formatNumberLikeClojure(args.reduce<number>((sum, v) => sum + parseNumber(v), 0));
  ...
  "currency-format": (n, locale, country) => {
    ...
    return new Intl.NumberFormat(loc, { style: "currency", currency }).format(n as number);
  },
  "number-format": (n, fmt, locale) => {
    throwWhenExpectingNumber(n);
    return numberFormat(n as number, String(fmt), locale ? String(locale) : undefined);
  },
  ...
  "double-format": (n, decimalPlaces) => {
    throwWhenExpectingNumber(n);
    return (n as number).toFixed(decimalPlaces == null ? 1 : parseNumber(decimalPlaces));
  },
  ...
  "get-digit": (n, i) => {
    ...
    if (idx < 0 || idx >= chars.length) return n;
...
for (const [name, fn] of Object.entries(builtIns)) filters.set(name, fn);

export function getFilter(name: string): FilterFunction | undefined {
  return filters.get(name.startsWith(":") ? name.slice(1) : name);
}

export function callFilter(name: string, ...args: unknown[]): unknown {
  const filter = getFilter(name);
  if (!filter) throw new Error(`No filter defined with the name '${name}'`);
  return filter(args[0], ...args.slice(1));
}

export function addFilter(name: string, fn: FilterFunction): void {
