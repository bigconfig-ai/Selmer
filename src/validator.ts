...
export function validateOn(): void {
  validationEnabled = true;
}
export const validateOn$ = validateOn;

export function validateOff(): void {
...
function loadErrorTemplate(): string {
...
export function formatTag(tag?: TagInfo): string {
  if (!tag) return "";
  if (tag.tagType === "expr") return `{% ${tag.tagName} ${tag.args?.join(" ") ?? ""} %}`;
  return `{{${tag.tagValue}}}`;
}

export function validationError(error: string, tag?: TagInfo, line?: number, template?: string | URL): never {
...
function validateFilters(template: string | URL, line: number, tag: TagInfo): void {
  const body = tag.tagType === "filter" ? tag.tagValue : "";
  for (const part of splitValue(body).slice(1)) {
    const filterName = part.split(":")[0].trim();
    if (filterName && !filters.has(filterName)) validationError(`Unrecognized filter ${body} found inside the tag`, tag, line, template);
  }
}
...
function validateTag(template: string | URL, line: number, stack: Array<TagInfo & { line: number }>, tag: TagInfo): Array<TagInfo & { line: number }> {
  if (tag.tagType === "filter") {
    validateFilters(template, line, tag);
    return stack;
  }

  for (const arg of tag.args) validateFilters(template, line, { tagType: "filter", tagValue: arg });
  const lastTag = stack[stack.length - 1];
  const endTags = lastTag?.tagType === "expr" ? closingTags.get(lastTag.tagName) ?? [] : [];

  if (!tag.tagName) validationError("No tag name supplied for the tag", tag, line, template);
  if (!closeTags().includes(tag.tagName) && !exprTags.has(tag.tagName)) validationError("Unrecognized tag found", tag, line, template);
  ...
  if ((closingTags.get(tag.tagName)?.length ?? 0) > 0) return [...stack, { ...tag, line }];
  return stack;
}

function skipVerbatim(scanner: Scanner, tag: TagInfo): void {
  if (tag.tagType !== "expr" || tag.tagName !== "verbatim") return;
  while (!scanner.eof()) {
    if (scanner.startsWith(scanner.delimiters.tagOpen + scanner.delimiters.tagSecond)) {
      const next = scanner.readTagInfo();
      if (next.tagType === "expr" && next.tagName === "endverbatim") return;
    } else scanner.readChar();
  }
}

export function validateTags(content: string, template: string | URL, opts: RenderOptions = {}): Array<TagInfo & { line: number }> {
  const scanner = new Scanner(content, makeDelimiters(opts));
  let line = 1;
  let stack: Array<TagInfo & { line: number }> = [];
  while (!scanner.eof()) {
...
export function validate(template: string | URL, opts: RenderOptions = {}): void {
  if (!validationEnabled) return;
  const content = readResource(template, opts).content;
  const orphanTags = validateTags(content, template, opts);
  if (orphanTags.length > 0) {
    validationError(
      `The template contains orphan tags: ${orphanTags.map((tag) => `${formatTag(tag)} on line ${tag.line}`).join(", ")}`,
      orphanTags[0],
      orphanTags[0].line,
      template
    );
